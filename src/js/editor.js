...
import sprite from 'url:../demo/assets/img/formeo-sprite.svg';
...
export class FormeoEditor {
  ...
  constructor({ formData, ...options }, userFormData) {
    ...
    const { actions, events, debug, config, editorContainer, ...opts } =
      mergedOptions;
    if (editorContainer) {
      this.editorContainer =
        typeof editorContainer === 'string'
  ...
  loadResources() {
    ...
    if (this.opts.svgSprite) {
      promises.push(
        ajax(this.opts.svgSprite, insertIcons, () =>
          ajax(FALLBACK_SVG_SPRITE, insertIcons)
        )
      );
    } else {
    ...
    promises.push(
      i18n.init(
        Object.assign({}, this.opts.i18n, {
          locale: sessionStorage.get(SESSION_LOCALE_KEY),
        })
      )
    );
